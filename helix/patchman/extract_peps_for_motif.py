...
def pose_from_wynton(pdbid,
    ...
    print('Opening from Wynton: {}'.format(fpath))
...
def extract_templates_for_motif(matches, pepseq, plen, patch, receptor_pose, scrfxn, design):
    ...
    for match in matches:
        match_idx += 1
        print('OPENING MATCH {} of {}'.format(match_idx, len(matches)))
        rmsd = match[0]
        motif_pdb = match[1][-6:-2]
        ...
        for i, stretch in enumerate(stretches):
            ...
            complex_name = patch_name + '_' + motif_pdb + '_' + str(line_idx) + '_%s' % str(stretch[0]) + '_%s' % str(stretch[-1]) + '.pdb'
            try:
